...
df_depth <- read.delim("depth.csv",sep = "\t")
...
The gene coverage for the different samples are visualized in a heatmap. The samples are displayed on the y-axis, and the reference sequences are shown on the x-axis. Higher gene coverage are represented by brighter yellow colors, while lower coverage are represented by darker blue tones.

```{r,heatmap}

df_long <- df_depth %>%
  select(Isolat, Gene, Coverage)
